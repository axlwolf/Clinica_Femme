...
require_once 'cms/util/Resource.php';
...
class ResourcesBundle extends Resource {
	...
	public function load( $filename , $language = null ) {
		$schemaFile = stream_resolve_include_path( 'cms/util/resourceBundle.xsd' );
		...
		if ( is_file( $filename ) ) {
			if ( is_file( $schemaFile ) ) {
				...
				if ( $dom->schemaValidate( $schemaFile ) ) {
					...
					$this->resourceXPath->registerNamespace( 'pro' , 'urn:CMS/pro' );
				} else {
